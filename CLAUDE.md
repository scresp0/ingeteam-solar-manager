...
- **Modos** (`main._compute_target_charge_current`):
  ...
  - **SOLAR** (08:00–fin de producción del forecast): carga a la **corriente mínima que llena la batería antes del fin de producción** = `amps_for(energía_pendiente, horas_hasta_fin)`, acotada `[floor_a, max_a]`. `amps_for` sube sola hacia `max_a` cuando falta tiempo o sobra energía, y baja al `floor_a` cuando sobra tiempo; el recálculo cada `interval_min` la reajusta si la carga se retrasa (nubes) — misma autocorrección que VALLE. Batería llena → no toca. **⚠️ Cambio v1.69:** se eliminó el acantilado "solar insuficiente → `max_a` (66)". Hasta v1.68, si la **producción restante NETA** (calibrada a p10 **y** restándole el consumo de casa vía `_remaining_solar_forecast`) no cubría el 100% del hueco, saltaba a 66A. En verano con AC ese neto es muy pequeño (medido 2026-07-12: 21 kWh p50 brutos restantes → 6.1 kWh netos tras p10 + `post_valley`≈22 kWh/día), así que por quedarse ~0.2 kWh corto forzaba 66A en pleno mediodía soleado — justo lo contrario del objetivo `calor ∝ corriente`. Ahora `remaining_solar_kwh` ya **no** entra en la decisión (solo se loguea en la línea `[CORRIENTE]`); `_compute_target_charge_current` dejó de recibirlo como parámetro. Pendiente aparte: `post_valley` incluye la tarde-noche sin sol pero se prorratea sobre las horas solares → sobre-resta consumo (no corregido, anotado).

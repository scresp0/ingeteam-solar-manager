...
`ARCHITECTURE.md` (raíz del repo) es el documento de diseño de referencia del proyecto: módulos, decisiones del sistema con sus fórmulas, referencia de `config.yaml`, integración con el inversor, gotchas e infraestructura. Debe actualizarse tras cualquier cambio estructural (nuevo módulo o job, cambio de algoritmo o de fórmulas, nuevos parámetros de config, cambios en la integración MODBUS/Playwright). No dupliques aquí su contenido: este fichero es el contexto operativo, aquel es la descripción de la arquitectura.
...
- **Disparo manual**: `POST /api/backup/run` (protegido con `web_api_key`) + botón "Lanzar backup ahora" dentro del grupo «Backup externo por SCP» de la pestaña Configuración (`runBackup()` en `index.html`). Usa la config ya guardada en disco, no lo que haya sin guardar en el formulario.
...
- **Editable desde la web desde v1.83**: la sección entera está en la lista blanca de `POST /api/config` y en el `CONFIG_SCHEMA` del form (no contiene secretos: la autenticación es por clave SSH montada). Hasta v1.82 solo se editaba a mano en `config.yaml`, aunque el botón de disparo manual ya vivía en esa misma pestaña.
...
- **Modos** (`main._compute_target_charge_current`):
  ...
  - **SOLAR** (08:00–fin de producción del forecast): **simulación franja a franja del excedente previsto** (v1.72). `_solar_surplus_window` construye el perfil `surplus[i]` = kWh que la batería podría absorber en cada franja de 30 min restante (producción p50·bias − consumo de la vivienda, suelo en 0); `_min_current_for_surplus` barre `I` de 1 a `max_a` y devuelve el primero cuyo `Σ min(I·V·0.5h, surplus[i])` alcanza `energía_pendiente · margin`. Acotada a `[floor_a, max_a]`. Batería llena → no toca.
    ...
    - **⚠️ Mezcla con perfil histórico por franja (v1.77):** la persistencia sola solo sabe lo que la casa ha consumido **hoy hasta ahora**, y `_solar_surplus_window` la aplicaba de forma PLANA a las ~26 franjas restantes del día — asumiendo implícitamente que el ritmo de la mañana se mantiene toda la jornada. Caso real que lo expuso: 2026-08-18, persistencia matutina 0,5-0,6 kW → controlador a `floor_a` (33A) toda la mañana pese a 21,5 kWh de excedente previsto → el AC dispara el consumo real de tarde a 1,1-2,3 kW → la batería se queda al 89%, sin llegar nunca al 100% (el sol se acaba antes de que el algoritmo reaccione). `_house_consumption_for_slot` mezcla ahora, franja a franja: peso 1 (pura persistencia) en la franja actual, decayendo lineal a peso 0 (puro histórico) a partir de `_HOUSE_PROFILE_BLEND_HOURS` (2h). El histórico es la **mediana** de `solar_media_hora.house_kwh` de esa franja de 30 min sobre los últimos `house_profile_window_days` días (`storage.get_house_power_profile`), con el mismo criterio conservador que el resto de dinámicos: si la franja peor cubierta tiene menos de `house_profile_min_days_in_window` días, `None` → fallback a pura persistencia (comportamiento idéntico a antes de v1.77). Restar el histórico como un TOTAL agregado del día (en vez de franja a franja) habría reintroducido el acantilado de v1.68/v1.69 — la producción es una campana, no una meseta, y el consumo tampoco es plano.
    - **`margin` subido de 1.2 a 1.33 (2026-08-18):** con el perfil histórico ya integrado, un backtest sobre el mismo día real (producción y consumo del datalogger como previsión perfecta, misma metodología que el backtest de v1.72) seguía sin llegar al 100% (99,4%). Se probaron dos alternativas: `margin=1.33` constante → **100% en el backtest**; margen decreciente (1.33 a las 08:00 → 1.0 al `solar_end` previsto) → solo 99,5%, descartado — decaía atado a `solar_end`, que la cola larga y fina del forecast de Solcast estira hasta ~21h aunque la producción real útil se acabe sobre las 19h, así que el margen ya se había consumido antes de que el riesgo real (el consumo de la tarde) hubiera pasado. El coste de `margin=1.33` en A·h totales del día es prácticamente neutro (incluso algo menor que con 1.2): al terminar de cargar antes, se ahorra el tramo final de BALANCE/rampa tardía que sí consume A·h en los demás escenarios.
    - **Instrumentación para un futuro `margin` dinámico (v1.78):** a diferencia de `risk_factor`/`solar_bias_factor`/`night_consumption_kwh`, no había ningún dato en InfluxDB que permitiera calibrar `margin` desde el histórico (esos tres ya tenían forecast-vs-real que comparar; para el margen no existía el "¿llegamos al objetivo, y por cuánto fallamos?"). `logger_reader._calculate_stats` añade dos campos a `stats_diarias`: `peak_soc_pct` (SOC máximo del día — de noche el SOC solo baja, así que el pico coincide con el momento justo tras la carga de valle+solar, sin necesidad de acotar una ventana horaria) y `battery_charged_kwh` (energía neta cargada, ∫`Pbatt`<0, valle+solar sin separar porque ambas cuentan para el mismo `max_soc_pct`). Comparar `battery_charged_kwh` contra el `Δ SOC × capacidad` del mismo día es, además, la forma de detectar el efecto de aceptación de carga que se vio en el backtest del 2026-08-18 (ratio ~1.09 en la franja 08:00-16:00, pero ~2.5 en 16:00-18:00 — posible caída de aceptación de carga cerca de SOC alto, no modelada ni aquí ni en el algoritmo). **Solo instrumentación — `charge_current.margin` sigue fijo en 1.33; no hay `get_dynamic_margin` todavía.** Con semanas de datos se podrá decidir si calibrarlo dinámicamente compensa frente a dejarlo fijo.
    - **Dos guards para no construir la ventana solar cuando el resultado se descarta (v1.79):** el tick llamaba a `_solar_surplus_window` (Solcast + `solar_bias` de InfluxDB + datalogger + perfil histórico de InfluxDB) en **cada** ejecución, 24h/día, aunque la decisión de esa hora nunca la usara — ~13h/día (valle + tarde-noche tras acabar la producción) de trabajo tirado. Verificado ejecutando `_compute_target_charge_current` con `window`/`solar_end` contradictorios en valle: mismo resultado exacto con `window=None` que con una ventana disparatada (la rama VALLE usa `_VALLE_END_HOUR` fijo; BALANCE-en-valle también, nunca el `solar_end` real). Dos guards, no uno, porque el criterio que sirve para cada caso es distinto:
      - **Valle** (`hour < _VALLE_END_HOUR`): guard por reloj en `run_charge_current_controller`, antes de llamar a `_solar_surplus_window` — cero riesgo. Ni siquiera se llama al fallback barato `_productive_window_end` (también InfluxDB): tan inútil en valle como la ventana completa.
      - **Tarde-noche**: el forecast NO sirve como criterio de "estamos en valle" — a las 02:00 el forecast de hoy todavía tiene sol por delante más tarde ese mismo día. Por eso este guard vive **dentro** de `_solar_surplus_window`: tras pedir `intervals` (ya el primer paso), si ninguna franja futura tiene `pv_estimate > 0` (exacto `0.0` en Solcast de noche, sin ambigüedad de `bias`), se corta ahí — antes de `get_dynamic_solar_bias`, `_house_power_estimate` (el datalogger, el caro) y `get_house_power_profile`.
      Probado con la función real monkeypatcheando las cuatro llamadas, confirmando 0 invocaciones con forecast a cero y 1 con forecast normal (`app/test_charge_current.py`).
...
### Pestaña Configuración (v1.47 · reescrita en v1.83)

**El invariante que hay que respetar al tocarla:** modelo Pydantic (`config.py`) ↔ lista blanca `_EDITABLE_FIELDS` (`web/server.py`) ↔ `CONFIG_SCHEMA` (`index.html`) describen el MISMO conjunto de campos. Nada las ataba y divergieron: en v1.82 el form no exponía ni un campo de `charge_current` (17, añadidos entre v1.53 y v1.79) ni de `backup` (11), ni `tariff.weekend_days`/`holidays` — 31 de 73 campos vivían solo en el YAML. `app/test_config_web.py` (botón "Test pestaña Configuración") fija esa correspondencia en las tres direcciones: **al añadir un parámetro a cualquier modelo el test falla** hasta exponerlo en la web o declararlo en `_EXCLUDED_FIELDS` con su motivo.

- **Form generado dinámicamente** desde `CONFIG_SCHEMA` en `index.html` (no hardcoded). Estructura: 6 **grupos colapsables** (decisión de carga · corriente de carga · planificación · integraciones · sistema · backup) → bloques (`section`, `title`, opcional `note`/`warn`) → `fields` con `key`, `label`, `type` (`number`/`text`/`bool`/`select`/`str_nullable`/`int_list`/`date_list`), atributos de validación HTML y `help` (texto de ayuda: la web sustituye al YAML como sitio de edición, y los comentarios del YAML no llegan al form).
- **Carga**: `loadConfigForm()` en init y en "Recargar desde disco". `GET /api/config` devuelve valores, `env_overrides`, `legacy_keys` y `default_keys`.
- **Valor efectivo, no `null`** (v1.83): para una clave ausente del YAML el backend resuelve el default del modelo (`_section_defaults`) y la marca en `default_keys` → badge gris `def`. Antes devolvía `null`: input vacío, guardado bloqueado con "valor numérico inválido", y parámetros que gobiernan el sistema (`temp_gate_enabled`, `house_profile_*`…) invisibles por no estar escritos en el fichero.
- **Guardado dirty-only** (v1.83): `_renderConfigForm` guarda un baseline y `_collectDirty()` envía SOLO los campos modificados. El botón muestra el número de cambios y cada grupo su contador. Antes se enviaban los 44 campos en cada guardado y, como `_cast_value` rechazaba `""` en los tipos `str`, un YAML con `mail_from: ""` (el caso recomendado: el valor vive en `.env`) hacía que cambiar `dry_run` devolviera `400 email.mail_from: valor vacío` **sin escribir nada**. Ahora `""` es válido en `str`; los campos donde vaciar rompería el arranque usan `str_required`.
- **Tipos nuevos** (v1.83): `time_hhmm` valida `HH:MM` antes de escribir (`tariff.schedule_at`, `backup.schedule_at`); `int_list` = `weekend_days` (chips L–D, se escribe en flow style para no ocupar tres líneas); `date_list` = `holidays` (chips con alta/baja; las fechas se escriben **entrecomilladas** o YAML las relee como `datetime.date`). `tariff.periods` sigue fuera: única estructura anidada, no cambia nunca.

...
- **Modos** (`main._compute_target_charge_current`):
  ...
  - **SOLAR** (08:00–fin de producción del forecast): se calcula la **producción solar restante de hoy calibrada** = Σ por franja de 30 min `(p10·rf + p50·(1−rf))·bias` con `rf`/`bias` **dinámicos** (`get_dynamic_risk_factor`/`get_dynamic_solar_bias`, fallback config) — el `rf` inclina hacia `p10` = margen de seguridad. **Si la producción restante ≥ energía pendiente** → mín. corriente para llenar con esa solar antes de que acabe la producción. **Si no** (solar insuficiente o sin forecast) → `max_a` (66) para llenar seguro. Batería llena → no toca.
    - **Puerta de temperatura (`temp_gate_enabled`, v1.55):** con `true` (default) la carga suave exige además `temp > hot_threshold_c` (batería fría → `max_a`). ⚠️ Esto es reactivo: solo carga despacio cuando ya hace calor, así que una mañana fría con sol de sobra cargaba a 66A todo el rato hasta que la batería cruzaba el umbral (caso real 2026-06-15: batería a 28-29°C toda la mañana → 66A pese a 22 kWh de sol por delante). Con `false` la temperatura no influye: carga al mínimo siempre que la solar baste (comportamiento *preventivo* — evita el calentamiento antes de que ocurra, alineado con el objetivo `calor ∝ corriente`).
